// ../.wrangler/tmp/bundle-zJblpU/checked-fetch.js
...
app.post("/renovate", async (c) => {
  ...
  const prompt = encodeURIComponent(
    `Architectural renovation of this exact space: ${command}. Photorealistic render. Maintain the exact same building structure, walls, roof shape, windows, doors, and camera angle. Only change materials, colors, textures and finishes as requested. High quality, detailed, professional architectural visualization.`
  );
  const seed = Math.floor(Math.random() * 999999);
  let url = `https://image.pollinations.ai/prompt/${prompt}?width=768&height=768&seed=${seed}&nologo=true&negative_prompt=blurry,low quality,deformed,ugly,changed structure,different building,different angle,different perspective,abstract,cartoon,sketch`;
  if (image) {
    url += `&image=${encodeURIComponent(image)}`;
  }
  try {
    console.log(`[POLLINATIONS] Requesting: seed=${seed}, hasImage=${!!image}`);
    const res = await fetch(url, { method: "GET" });
    if (!res.ok) {
      const text = await res.text();
      return c.json({ success: false, error: `Pollinations ${res.status}: ${text.slice(0, 200)}` }, 500);
    }
    const blob = await res.arrayBuffer();
    const base64 = arrayBufferToBase64(blob);
    const dataUrl = `data:image/png;base64,${base64}`;
    console.log(`[POLLINATIONS] Success: ${base64.length} chars base64`);
    return c.json({ success: true, resultUrl: dataUrl, model: "pollinations" });
  } catch (err) {
    return c.json({ success: false, error: err.message || "AI failed" }, 500);
  }
});
var onRequest = (context) => app.fetch(context.request, context.env);

// ../.wrangler/tmp/pages-5RDMss/functionsRoutes-0.4874856245099215.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-zJblpU/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-zJblpU/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.494285034734387.mjs.map
