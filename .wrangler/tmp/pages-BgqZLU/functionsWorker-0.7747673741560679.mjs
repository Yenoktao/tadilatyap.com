// ../.wrangler/tmp/bundle-aCqFRt/checked-fetch.js
...
app.get("/debug", async (c) => {
  const t = c.env.REPLICATE_API_TOKEN;
  return c.json({ ok: true, hasToken: !!t, tokenLength: t?.length || 0 });
});
...
app.post("/renovate", async (c) => {
  ...
  const input = {
    prompt: `Renovate this space: ${command}. Photorealistic architectural visualization. Maintain the exact same room structure, walls, layout, and perspective. Only change materials, colors, textures and finishes as requested.`,
    num_outputs: 1,
    aspect_ratio: "1:1",
    output_format: "png",
    output_quality: 80,
  ...
  try {
    ...
    if (p.status === "succeeded" && p.output) {
      ...
      if (Array.isArray(p.output) && p.output.length > 0)
        ...
        url = p.output;
      if (url)
        return c.json({ success: true, resultUrl: url, model });
    }
    const id = p.id;
    if (id) {
      for (let i = 0; i < 20; i++) {
        ...
        if (d.status === "failed" || d.status === "canceled")
          return c.json({ success: false, error: `Prediction ${d.status}` }, 500);
      }
    }
...
// ../.wrangler/tmp/pages-BgqZLU/functionsRoutes-0.045357797524826715.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-aCqFRt/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-aCqFRt/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7747673741560679.mjs.map
