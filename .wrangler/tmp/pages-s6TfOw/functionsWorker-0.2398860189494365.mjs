// ../.wrangler/tmp/bundle-dsDW34/checked-fetch.js
...
app.post("/renovate", async (c) => {
  const cfToken = c.env.REPLICATE_API_TOKEN;
  const cfAccountId = c.env.CF_ACCOUNT_ID;
  if (!cfToken || !cfAccountId) {
    return c.json({ success: false, error: "REPLICATE_API_TOKEN veya CF_ACCOUNT_ID eksik" }, 500);
  }
  ...
  try {
    ...
    if (!prompt || typeof prompt !== "string") {
      return c.json({ success: false, error: "prompt zorunlu" }, 400);
    }
    if (!imageFile || typeof imageFile === "string") {
      return c.json({ success: false, error: "image zorunlu (dosya)" }, 400);
    }
    ...
    if (!aiResponse.ok) {
      ...
      return c.json({ success: false, error: `Cloudflare AI ${aiResponse.status}: ${errText.slice(0, 200)}` }, 502);
    }
    const outBuffer = await aiResponse.arrayBuffer();
    const base64 = arrayBufferToBase64(outBuffer);
    const dataUrl = `data:image/png;base64,${base64}`;
    return c.json({ success: true, resultUrl: dataUrl, model: "cf-stable-diffusion-img2img" });
  } catch (err) {
...
// ../.wrangler/tmp/pages-s6TfOw/functionsRoutes-0.5712872313530177.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-dsDW34/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-dsDW34/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.2398860189494365.mjs.map
