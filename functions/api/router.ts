...
async function callReplicate(
  ...
  if (!createRes.ok) {
    ...
    throw new Error(err.detail || `Failed to create prediction: ${createRes.status}`);
  }
  ...
  if (!result) throw new Error("Prediction timed out after 60s");
...
const renovateRouter = createRouter({
  generate: publicProcedure
    .input(z.object({ imageUrl: z.string().url(), command: z.string().min(1) }))
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        return { success: false, resultUrl: null, error: error.message || "AI generation failed" };
      }
    }),

  generateFromPrompt: publicProcedure
    .input(z.object({ command: z.string().min(1), metrekare: z.number().optional() }))
    .mutation(async ({ input, ctx }) => {
      try {
        const prompt = `Premium interior renovation visualization: ${input.command}. ${input.metrekare ? `Space approximately ${input.metrekare} square meters.` : ""} Photorealistic architectural photography, 8K quality, modern luxury design.`;
